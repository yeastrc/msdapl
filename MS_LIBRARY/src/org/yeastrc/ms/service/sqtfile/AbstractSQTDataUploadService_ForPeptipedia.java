...
/**
 * !!!! ---------------------------------------------------------------------------
 * !!!! This file is only to be used for uploading Tide results for the 
 * !!!! YRC's Peptipedia project.
 * !!!! Search for "PEPTIPEDIA" to find places where code had to be added to 
 * !!!! get the some Tide output files uploaded. 
 * !!!! The problem arises because Tide cannot deal with Bullseye processed data
 * !!!! that can have multiple, slightly differing, precursor masses for the 
 * !!!! same MS/MS scan.
 * !!!! ---------------------------------------------------------------------------
 */

abstract class AbstractSQTDataUploadService_ForPeptipedia implements SearchDataUploadService {

    static final Logger log = Logger.getLogger(AbstractSQTDataUploadService_ForPeptipedia.class);
    ...
    public AbstractSQTDataUploadService_ForPeptipedia() {
...
//                return false;
        ...
        // Added this for failed PEPTIPEDIA uploads due to data processes with Bullseye -- Tide cannot deal with 
        // multiple experimental mass values for the same scan
        try {
        	uploadSearchScan(new SQTSearchScanWrap(scan, runSearchId, scanId));
        }
        catch(UploadException e) {
        	
        	// If this result already exists in our cache we will not upload it again
        	// This will not solve the problem of a duplicate result already in the 
        	// database.  
        	if(e.getErrorCode() == ERROR_CODE.DUPLICATE_SCAN_CHARGE) {
        		return false; 
        	}
        	else {
        		throw e;
