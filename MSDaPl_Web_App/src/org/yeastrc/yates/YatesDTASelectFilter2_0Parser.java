/*
 * YatesDTASelectFilter2_0Parser.java
...
public class YatesDTASelectFilter2_0Parser  implements
	...
	public List parseFile(BufferedReader br, int databaseID) throws Exception {
		...
		while ((line = br.readLine()) != null) {
			...
			if (fields.length == 9 || (fields.length > 2 && fields[1].equals("Proteins"))) {
				...
				if (fields[0].equals("Unique")) {
					if (fields[9].equals( "Zscore" ))
						isZscore = true;
				...
				pep.setConfPercent(Double.parseDouble(fields[4]));
				pep.setMH(Double.parseDouble(fields[5]));
				pep.setCalcMH(Double.parseDouble(fields[6]));
				pep.setTotalIntensity(Double.parseDouble(fields[7]));
				pep.setSpRank(Integer.parseInt(fields[8]));
				
				if (isZscore)
					pep.setZScore(Double.parseDouble(fields[9]));
				else
					pep.setSpScore(Double.parseDouble(fields[9]));

				pep.setIonProportion(Double.parseDouble(fields[10]));
				pep.setRedundancy(Integer.parseInt(fields[11]));
				pep.setSequence(fields[12]);
